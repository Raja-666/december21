...
const ProfileModal = require('../models/Profileschema');
...
const handleGetCollections = async (req, res) => {
  try {
    const collections = await Collection.find({ status: {$in:[1,2,3,0]} }, { logo: 1, collectionName: 1 , status:1 });
...
const handleGetNftDetail = async (req, res) => {
  try {
    ...
    const colData = await Collection.findOne({ _id: nftData.collection }, {  });
    const userData = await ProfileModal.findOne({ address: nftData.address }, {  });

    showData['uploadfile'] = nftData.uploadfile
    showData['_id'] = nftData._id
    showData['nftName'] = nftData.nftName
    showData['traits'] = nftData.traits
    showData['collectionName'] = colData.collectionName
    showData['collectionLogo'] = colData.logo
    showData['description'] = colData.description
    showData['userName'] = userData.userName
    showData['profileImg'] = userData.profileImg
    showData['owner'] = nftData.address

    console.log('colData', colData)
    // nftData['collectionName'] = colData.collectionName
    res.status(200).json({ success: true, data: showData, message: 'Successfully get!!' });
  } catch (error) {
    console.error(error);
    console.log('fdfs', error)
    res.status(500).json({ success: false, data: {}, message: 'Err-' + error.message });
...
module.exports = { handleCreateCollection, handleListCollection, handleGetCollections, handleViewCollections, handleGetNftDetail };